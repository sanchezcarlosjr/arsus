...
export class RssComponent implements OnInit {
  ...
  sources = this.store.selectOnce(AuthStateModule.uid).pipe(
    switchMap((userUID: string) => {
      ...
      return this.firestore.collection('sources', (query) => query.where('subscribers', "array-contains", userUID)).valueChanges()
    })
  )
  ...
  remove(uid: string) {
    this.firestore.collection('sources').doc(uid).update({
